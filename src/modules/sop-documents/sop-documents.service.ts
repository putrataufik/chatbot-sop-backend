...
export class SopDocumentsService {
  ...
  async findById(id: number): Promise<any> {
    const sop = await this.sopRepository.findOne({ where: { id }, relations: ['uploaded_by_user'] });
    if (!sop) throw new NotFoundException(`Dokumen id ${id} tidak ditemukan`);
    const { uploaded_by_user, ...rest } = sop;
    return { ...rest, uploaded_by: { id: uploaded_by_user.id, name: uploaded_by_user.name } };
  }

  async update(id: number, title: string): Promise<{ message: string }> {
    const sop = await this.findById(id);
    const isExist = await this.sopRepository.findOne({ where: { title, id: Not(sop.id) } });
    if (isExist) throw new BadRequestException(`Judul "${title}" sudah digunakan.`);
  ...
  async findAllMetadata(): Promise<Array<{ id: number; title: string; file_size: number }>> {
